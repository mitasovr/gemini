...
module.exports = class BrowserRunner extends Runner {
    ...
    run(suiteCollection, stateProcessor) {
        ...
        return _(suites)
            ...
            .map((suite) => {
                if (!suite.metaInfo || !suite.metaInfo.url) {
                    suite.metaInfo = suite.metaInfo || {};
                    _.extend(suite.metaInfo, {url: this._mkFullUrl(suite.url)});
                }
    ...
    _runSuite(suite, stateProcessor) {
        ...
        const runner = SuiteRunner.create(suite, browserAgent, this._browserConfig);
